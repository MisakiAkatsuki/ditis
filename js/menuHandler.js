...
function setupContextMenu() {
    ...
    spreadsheet.addEventListener('contextmenu', (e) => {
        ...
        // フレームヘッダーの上で右クリックした場合
            ...
            // 行ヘッダー用メニュー
            menuItems = [
                { action: 'insert-here', label: '行を挿入' },
                { action: 'delete-frames', label: '行を削除' },
                { action: 'toggle-disable-frames', label: '行を無効化/有効化' },
                { divider: true },
                { action: 'shift-down-from-here', label: 'コマをシフト' },
                { divider: true },
                { action: 'copy', label: '行の値をコピー' },
                { action: 'cut', label: '行の値をカット' },
                { action: 'paste', label: '行の値にペースト' }
            ];
        }
        // 列ヘッダーの上で右クリックした場合
            ...
            // 列ヘッダー用メニュー
            menuItems = [
                { action: 'insert-column', label: '列を挿入' },
                { action: 'rename-layer', label: 'レイヤー名を変更' },
                { action: 'swap-column-left', label: '左の列と入れ替え' },
                { action: 'swap-column-right', label: '右の列と入れ替え' },
                { action: 'delete-column', label: '列を削除' },
                { action: 'delete-columns-after', label: 'この列以降を削除' },
                { divider: true },
                { action: 'delete-content', label: 'セル内容を削除' },
                { divider: true },
                { action: 'copy', label: '列の値をコピー' },
                { action: 'cut', label: '列の値をカット' },
                { action: 'paste', label: '列の値にペースト' }
            ];
        }
        else {

...
/**
 * タブを描画する（旧実装 - 現在は使用していない）
 * - 全シートのタブを描画
 * - アクティブタブのハイライト
 * - タブのドラッグ&ドロップイベントを設定（イベント委譲）
 */
function renderTabsOld() {
    const container = document.getElementById('tabs-container');
    
    // 初回のみイベント委譲を設定
    if (!container._tabEventsSetup) {
        container._tabEventsSetup = true;
        ...
        // ドラッグイベントはapp.jsのrenderTabs()で個別に設定されるため、ここではコメントアウト
        /*
        container.addEventListener('dragstart', (e) => {
            const tab = e.target.closest('.tab');
            if (tab) handleTabDragStart(e);
        });
        
        container.addEventListener('dragover', (e) => {
            const tab = e.target.closest('.tab');
            if (tab) handleTabDragOver(e);
        });
        
        container.addEventListener('drop', (e) => {
            const tab = e.target.closest('.tab');
            if (tab) handleTabDrop(e);
        });
        
        container.addEventListener('dragend', (e) => {
            const tab = e.target.closest('.tab');
            if (tab) handleTabDragEnd(e);
        });
        */
    }
    ...
    AppState.sheets.forEach((sheet, index) => {
        ...
        tab.className = 'tab' + (index === AppState.currentSheetIndex ? ' active' : '');
        tab.draggable = true;
        tab.dataset.index = index;
        tab.innerHTML = `
            <span class="tab-name">${escapeHtml(sheet.name)}</span>
            ${AppState.sheets.length > 1 ? '<span class="tab-close">×</span>' : ''}
        `;
...
function handleTabDragStart(e) {
    ...
    AppState.draggedTabIndex = parseInt(e.target.dataset.index);
 ...
 * タブのドラッグオーバー処理
 * @param {DragEvent} e - ドラッグイベント
 * @returns {boolean} - false（デフォルト動作をキャンセル）
 */
...
function handleTabDrop(e) {
    ...
    const dropIndex = parseInt(e.target.closest('.tab').dataset.index);
    const dragIndex = AppState.draggedTabIndex;
    
    if (dragIndex !== dropIndex) {
        // タブを入れ替え
        const draggedSheet = AppState.sheets[dragIndex];
        AppState.sheets.splice(dragIndex, 1);
        AppState.sheets.splice(dropIndex, 0, draggedSheet);
        
        // 現在のシートインデックスを更新
        if (AppState.currentSheetIndex === dragIndex) {
            AppState.currentSheetIndex = dropIndex;
        } else if (dragIndex < AppState.currentSheetIndex && dropIndex >= AppState.currentSheetIndex) {
    ...
    for (let frame = renderStartRow; frame <= renderEndRow; frame++) {
        ...
        sheet.layers.forEach((layer, layerIndex) => {
            ...
            if (specialInfo.isCrossMark && value === '') {
                ...
                const cellHeight = parseInt(computedStyle.getPropertyValue('--cell-height'));
                const actualWaveHeight = cellHeight * 3; // SVGの実際の高さに合わせる
                const relativePosition = frame - 2;
                const absolutePosition = relativePosition * cellHeight;
